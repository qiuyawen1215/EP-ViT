...
class EventfulTokenwiseBlock(Block):
    ...
    def __init__(self, gate_before_ln=False, stgt=False, **super_kwargs):
        ...
        token_gate_class = SimpleSTGTGate if stgt else TokenGate    #默认token gate
    ...
    def forward(self, x):
        if self.exchange_index is not None and self.force_index is not None:
            self.force_index = self.force_index[~torch.isin(self.force_index, self.exchange_index)].unsqueeze(0)
        self.qkv_accumulator.modify_selfb_by_mvs(self.mvs, self.exchange_index)
        self.projection_accumulator.modify_selfb_by_mvs(self.mvs, self.exchange_index)
        self.mlp_accumulator.modify_selfb_by_mvs(self.mvs, self.exchange_index)
    ...
    def _forward_post_attention(self, x, skip_1):
        x, index = self.projection_gate(x, forced_index=self.force_index)
        x = self.projection(x)
        ...
        if self.gate_before_ln:
            x, index = self.mlp_gate(x, forced_index=self.force_index)
            x = self.mlp_layer_norm(x)
        else:
            x = self.mlp_layer_norm(x)
            x, index = self.mlp_gate(x, forced_index=self.force_index)
    ...
    def _forward_pre_attention(self, x):
        ...
        if self.gate_before_ln:
            x, index = self.qkv_gate(x, forced_index=self.force_index)
            x = self.input_layer_norm(x)
        else:
            x = self.input_layer_norm(x)
            x, index = self.qkv_gate(x, forced_index=self.force_index)
...
class EventfulMatmul1Block(EventfulTokenwiseBlock):
    ...
    def _forward_matmul_1(self, x):
        ...
        self.matmul_accumulator_1.modify_selfb_by_mvs(self.mvs, self.exchange_index)
