...
class TokenBuffer(ExtendedModule):
    ...
    def modify_selfb_by_mvs(self, mvs, exchange_index):
        if self.b is not None and exchange_index is not None:
            index = exchange_index
            self.b_new = self.b.clone()
            valid_index = (index + mvs[index]).clamp(0, self.b.shape[1] - 1)
            self.b_new[:, index, :] = self.b[:, valid_index, :]
            self.b = self.b_new
            del self.b_new
...
class MatmulBuffer(ExtendedModule):
    ...
    def modify_selfb_by_mvs(self, mvs, exchange_index):
        if self.product is not None and exchange_index is not None:
            index = exchange_index
            self.b_new = self.product.clone()
            valid_index = (index + mvs[index]).clamp(0, self.product.shape[1] - 1)
            self.b_new[:, :,index, :] = self.product[:, :,valid_index, :]
            self.product = self.b_new
            del self.b_new
